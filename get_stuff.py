...
def get_issue_numbers_from_issue_body(issue_number):
    issue_body = GITHUB_REPO_OBJECT.get_issue(issue_number).body
...
def get_epic_issue_numbers(epic_issue_number):
    ...
    epic_json_data = requests.request(method='GET', url=epic_issues_request_url, headers=head, verify=False)
